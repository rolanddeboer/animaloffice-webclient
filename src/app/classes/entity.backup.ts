...
export class Entity {
  ...
  setData( data: any[], params: EntityParams ): void
  {
    ...
    let defaultSort = "";
    if ( "defaultSort" in params ) {
      defaultSort = params.defaultSort;
      if (!( "sortKeys" in params )) params.sortKeys = [];
      params.sortKeys.push( defaultSort );
    }

    this.deletedField = ( "deletedField" in params ) ? params.deletedField: "";
    const sortKeys = ( "sortKeys" in params ) ? params.sortKeys: [];

    this.makeSorts( sortKeys, defaultSort );

    // if ( "sortKeys" in params ) {
    //   this.makeSorts( params.sortKeys, defaultSort );
    // }
  ...
  private makeSorts( sortKeys: string[], defaultSort = "", deletedField = "" ): void
  {
    if ( !sortKeys.length ) {
      if ( deletedField ) {
        for ( let key of Object.keys( this.data ) ) {
          this.sorts["active"]["unsorted"] = key;
        }
      }
      return;
    }
    ...
    for ( let sortKey of sortKeys ) {
      this.sorts["all"][ sortKey ] = [];
      this.sorts["active"][ sortKey ] = [];
    }

    // fill each sort key array with the corresponding data values
    for ( let key of Object.keys( this.data ) ) {
      for ( let sortKey of sortKeys ) {
        this.sorts["all"][ sortKey ].push( [
          this.data[key][sortKey],
          key
        ]);
        if ( deletedField && !this.data[key][deletedField]) {
          this.sorts["active"][ sortKey ].push( [
            this.data[key][sortKey],
            key
          ]);
        }
    ...
    for ( let sortKey of sortKeys ) {
      if ( sortKey != defaultSort ) {
        this.doSort( "all", sortKey );
      }
      if ( deletedField ) {
        this.doSort( "active", sortKey );
      } else {
        this.sorts[ "active" ][ sortKey ] =  this.sorts[ "all" ][ sortKey ];
      }
    }
  }

  private doSort( type: string, sortKey: string ): void
  {
    ...
    this.sorts[ type ][ sortKey ].sort( sortingFunction );
    const sortedKeys = [];
    for ( let item of this.sorts[ sortKey ] ) {
      sortedKeys.push( item[1] );
    }
    this.sorts[ type ][ sortKey ] = sortedKeys;
  }
